...
SELECT
    ...
    total_royalties.total_royalties+total_royalties.advance AS profit
FROM
    (SELECT 
        royalties.title_id,
        royalties.au_id,
        SUM(royalties.sales_royalty) as total_royalties,
        SUM(royalties.advance
        FROM
            (SELECT 
                sales.title_id,
                t.au_id, 
                (t.price*sales.qty*(t.royalty/100)*(t.royaltyper/100)) AS sales_royalty,
                t.advance
            FROM
                sales 
                LEFT JOIN 
                    ...
                    FROM
                        ...
                        LEFT JOIN
                            titles
                        ON titleauthor.title_id = titles.title_id) AS t
                ON
                    sales.title_id = t.title_id) AS royalties
        GROUP BY royalties.title_id, royalties.au_id) AS total_royalties
    ...
    FROM
        ...
        FROM
            ...
            FROM
                ...
                ON titleauthor.title_id = titles.title_id) AS t
                LEFT JOIN sales
                ON t.title_id = sales.title_id) as royalties
            GROUP BY royalties.title_id) as total_royalties
        GROUP BY total_royalties.au_id) as profit
    ORDER BY profit DESC
LIMIT 3;'''
